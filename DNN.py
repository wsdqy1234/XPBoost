...
class DNNModel(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, args):
        ...
        if args.modelName == 'DNN':
            self.model = DNN(args, self.dimI, self.dimO).to(torch.device('cuda:{}'.format(args.device)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataName', type=str, default='exchange')
    ...
    parser.add_argument('--nodes', default=[128])

    parser.add_argument('--window', type=int, default=1, help='window size')
    parser.add_argument('--clip', type=float, default=10., help='gr adient clipping')
