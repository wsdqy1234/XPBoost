...
from models.DCTCN import DCTCN
from utils import mkdir
import os.path as osp


class DCTCNModel(BaseEstimator, RegressorMixin):

    def __init__(self, args):

        super(DCTCNModel, self).__init__()
        ...
        if args.modelName == 'DCTCN':
            self.model = DCTCN(args, self.dimI, self.dimO).to(torch.device('cuda:{}'.format(args.device)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelName', type=str, default='DCTCN')
    parser.add_argument('--dimFC', type=int, default=64, help='number of FC units')

    parser.add_argument('--dimC', type=int, default=100, help='number of CNN hidden units')
