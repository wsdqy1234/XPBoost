...
class TPALSTMModel(BaseEstimator, RegressorMixin):

    def __init__(self, args):

        super(TPALSTMModel, self).__init__()
        ...
        if args.modelName == 'TPALSTM':
            self.model = TPALSTM(args, self.dimI, self.dimO).to(torch.device('cuda:{}'.format(args.device)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelName', type=str, default='TPALSTM')
    ...
    parser.add_argument('--hidLSTM', type=int, default=100, help='number of RNN hidden units')
